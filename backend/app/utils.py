...
def analyse_activity(events : list[GitHubEvent]) -> ActivityInsights:
    ...
    return ActivityInsights(most_active_day = most_active_day,
        ...
        recent_events = most_recent_events)

#This is again for past month activity
def calculate_collaboration_score(events: list[GitHubEvent], username: str, repos: list[GitHubRepo]) -> CollaborationScore:
    ...
    for event in events:
        ...
        if not is_own_repo:
            ...
            multiplier = 0.1       #Solo - Owned by user and is original work
        ...
        if event_type == "PullRequestEvent":
            ...
            if payload.pull_request and payload.pull_request.merged:    # Merged check
                social_points_points += (20 * multiplier)
